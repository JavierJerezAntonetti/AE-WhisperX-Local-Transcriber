...
- **AI-Powered Sentence Splitting:** Optional integration with Google Gemini 3.6 Flash to intelligently split long sentences into short, captionable chunks (max 9 words per segment).
...
6.  **Transcribe:** Click "Select Audio File & Start Transcription".
    ...
    - The API transcribes the audio using WhisperX:
      ...
      - **Sentence Level:** Returns sentence-level segments. If a Gemini API key is provided, uses Gemini 3.6 Flash to split long sentences into shorter, captionable chunks (max 9 words each).
